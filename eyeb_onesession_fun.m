%dd = pwd;
%dd = [dd '/'];
function onesession = eyeb_onesession(session_directory)
    if ~strcmp(session_directory(end), '/')
        dd = [session_directory '/'];
    else
        dd = session_directory
    end
...
%     end;
    ...
    clearvars -except mousepath videoanalysispath folders failed foldername
    close all
    catch exception
        failed = [failed; {date}];
